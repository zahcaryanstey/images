...
def train(model,loader,criterion,optimizer,config):
    ...
    for epoch in range(config.num_epochs):
        for batch_idx,(img,y_label) in enumerate(loader):

            loss = train_batch(img,y_label,model,optimizer,criterion)
            example_ct += len(img)
            batch_ct += 1
            # report metrics every 25th batch
            if((batch_ct + 1 ) % 25) == 0:
                train_log(loss,example_ct,epoch)
def train_batch(img,y_label,model,optimizer,criterion):
    img, y_label = img.to(device), y_label.to(device)
    # forward pass
    outputs = model(img)
...
# Run the model on some test examples
    with torch.no_grad():
        correct,total = 0,0
        for img,y_label in test_loader:
